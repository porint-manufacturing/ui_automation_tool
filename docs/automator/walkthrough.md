...
- **Action**:
  ...
  - `Click`: クリック
    - **検証機能**: クリック位置の要素が指定したRPA_PATHと一致するか自動検証します。不一致の場合は警告ログを出力します。
  - `Input`: テキスト入力 (`Value`にテキスト)。`{変数名}` で変数の値を参照可能。
    - **レガシーアプリ対応**: フォーカス設定後、最大1秒間フォーカス確立を待機してから入力します。
  - `Wait`: 待機 (`Value`に秒数)
  ...
  - `SetVariable`: 変数を設定 (`Value`に `name = value`)。
    - **算術演算**: 数式を評価できます。例: `count = {count} + 1`, `total = {price} * {quantity}`
    - **0埋め対応**: `04 + 1` のような0埋めされた数値も正しく計算されます（結果: `5`）。
  ...
  - `Invoke`: 要素を `InvokePattern` で実行します。ボタンクリックなどに使用します。`Click` より安定している場合があります。
    - **レガシーアプリ対応**: フォーカス設定後、最大1秒間フォーカス確立を待機してから実行します。
...
3.  **エラーメッセージでの表示**:
    - エイリアスを使用している場合、エラーや警告メッセージにはエイリアス名とRPA_PATHの両方が表示されます。
    - 例: `Element not found for key: 'Btn_Calc_5' (ButtonControl(Name='5'))`
...
古いWindowsアプリケーションを操作する際の留意事項です。

### 6.1. フォーカス処理の強化

レガシーアプリでは、フォーカス設定に時間がかかる場合があります。以下のアクションでは自動的にフォーカス確立を待機します:

- **Input**: `SetFocus()` 後、最大1秒間 `HasKeyboardFocus` をチェックし、フォーカス確立を確認してから入力します。
- **SendKeys**: 同様にフォーカス確立を待機してからキー送信します。
- **Invoke**: フォーカス確立を待機してから `InvokePattern` を実行します。
- **FocusElement**: 最大1秒間フォーカス確立を待機し、結果をログに記録します。

### 6.2. Click検証機能

`Click` アクションでは、クリック位置の要素が指定したRPA_PATHと一致するか自動検証します。不一致の場合は警告ログを出力しますが、実行は継続されます。

**警告例:**

```
Element mismatch at cursor position!
  Expected: 'OKButton' (Button[Name:OK])
  At cursor: Pane[Name:Overlay]
  This may indicate the element is obscured or the position is incorrect.
```

### 6.3. トラブルシューティング

- **フォーカスが移動しない**: 要素が `IsKeyboardFocusable=false` の場合、フォーカスは移動しません。ログで確認してください。
- **入力が別の要素に行く**: `FocusElement` で明示的にフォーカスを設定してから `Input` を実行してください。
- **クリック位置がずれる**: High DPI環境では自動調整されますが、問題がある場合は `Invoke` アクションを試してください。
